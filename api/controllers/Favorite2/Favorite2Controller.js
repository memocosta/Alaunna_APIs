...
module.exports = {
  index: async function (req, res) {
    try {
      ...
      SelectedCart = await Favorite2.findOne(filterOBJ);
      if (SelectedCart) {
        Selectedproducts = await FavoriteProducts.findAll({
          where: {
      ...
      return ResponseService.SuccessResponse(res, 'success for get the selecteing favorites', responsedata);
    } catch (err) {
      console.log(err);
      return ResponseService.ErrorResponse(res, 'somthing wrong happen when get the favorites', {
        action: 'unkwon-error',
        error: err
      });
    }
  ...
  index_market: async function (req, res) {
    try {
      ...
      filterOBJ['order'] = [['productsData', 'owner', 'desc']];
      SelectedCart = await Favorite2.findOne(filterOBJ);
      if (SelectedCart) {
        Selectedproducts = await FavoriteProducts.findAll({
          where: {
  ...
  create: async function (req, res) {
    try {
      ...
      let SelectedCart = await Favorite2.findOne({
        where: {
      ...
      if (SelectedCart) {
        ...
        for (let i = 0; i < params.products.length; i++) {
          let SelectedCartProducts = await FavoriteProducts.findOne({
            where: {
              cart_id: SelectedCart.id,
        ...
        return ResponseService.SuccessResponse(res, 'success for Updating the favorites successfully', params);
        ...
        let CreatedCart = await Favorite2.create(CartParam);
        for (let i = 0; i < params.products.length; i++) {
          await FavoriteProducts.create({
            quantity: params.products[i].quantity,
            product_id: params.products[i].product_id,
        ...
        return ResponseService.SuccessResponse(res, 'success for creating the favorites successfully', params);
      }
    } catch (err) {
      console.log(err);
      return ResponseService.ErrorResponse(res, 'somthing wrong happen when create the favorites', {
        action: 'unkown-error',
        error: err
      })
    }
  ...
  removeCartProduct: async function (req, res) {
    try {
      ...
      let filterOBJ = {where: {}};
      let SelectedCartProduct = await FavoriteProducts.findOne({where: {product_id: product_id}});
      filterOBJ['where']['owner_id'] = owner_id;
      filterOBJ['where']['active'] = 1;
      let SelectedCart = await Favorite2.findOne(filterOBJ);
      console.log(SelectedCart)
      if (SelectedCartProduct.quantity > 1) {
        ...
        SelectedCart.save();
        return ResponseService.SuccessResponse(res, 'success for Update the favorites Product', {});
      } else {
        ...
        SelectedCart.total_price = SelectedCart.total_price - price;
        SelectedCart.save();
        return ResponseService.SuccessResponse(res, 'success for remove the favorites Product', {});
  ...
  changeCartStatus: async function (req, res) {
    try {
      ...
      return ResponseService.SuccessResponse(res, 'success for change favorites status', {});

    } catch (err) {
      return ResponseService.ErrorResponse(res, 'somthign wrong happen when change favorites status ', {
        action: 'unkown-err',
        error: err
      })
    }
